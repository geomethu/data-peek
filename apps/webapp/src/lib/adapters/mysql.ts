...
export class MySQLWebAdapter implements WebDatabaseAdapter {
  ...
  async connect(creds: ConnectionCredentials): Promise<void> {
    ...
    this.threadId = (rows as { id: number }[])[0]?.id ?? null;
  }
  ...
  async query(sql: string, timeoutMs = 30000): Promise<WebQueryResult> {
    ...
    const resultRows = Array.isArray(rows)
      ...
      : [];
    const resultFields: QueryField[] = (fields || []).map(
      (f: mysql.FieldPacket) => ({
        name: f.name,
  ...
  async cancelQuery(): Promise<void> {
    if (!this.threadId || !this.connectionConfig) return;
  ...
  async getColumnStats(
    ...
    const distinctCount =
      Number((countRows as CountRow[])[0]?.distinct_count) || 0;
...
function buildMySQLSchemaInfo(
  ...
  for (const r of routines) {
    ...
    if (!schema) continue;
    const rParams =
      paramMap.get(`${r.routine_schema}.${r.specific_name}`) ?? [];
